...
public class GvrTrackerManager : MonoBehaviour
{
    #region Singleton
    private static GvrTrackerManager instance;

    public static GvrTrackerManager Instance
    {
        get { return instance; }
    }
    #endregion // Singleton
    ...
    private void Update()
    {
        if (trackingCamera != null)
        {
            ...
            trackingCamera.transform.localRotation = UnityARMatrixOps.GetRotation(matrix);//InputTracking.GetLocalRotation(XRNode.CenterEye);   // RotationはXRのトラッキング情報を使用
        }
    }
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        if(trackingCamera == null)
        {
            ...
            // Cardboardの自動ヘッドトラッキング停止
            UnityEngine.XR.XRDevice.DisableAutoXRCameraTracking(trackingCamera, true);
            // session.RunWithConfigAndOptions(config, UnityARSessionRunOption.ARSessionRunOptionResetTracking);
        }
    }
